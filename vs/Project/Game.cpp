...
mat4 Game::CreateDefaultProjection()
{
	...
	//akan mengembalikan nilai ortho, nilai (0, 0) boleh ditengah, kiri bawah, kiri atas
}
...
void Game::GameLoop(SDL_Window* window)
{
	...
	while (State::RUNNING == state) {
		...
		Update();
		//Setting Viewport
		glViewport(0, 0, setting->screenWidth, setting->screenHeight); //full layar, klo mau gk full bisa ubah nilai (0,0)
...
void Game::PollInput()
{
	SDL_Event evt; //menangkap event-event yang ada di SDL
	inputManager->Reset();
