...
void Engine::Text::Draw()
{
	...
	// Activate corresponding render state (mengaktifkan transparansi)
	glEnable(GL_BLEND);
	...
	{
		Character ch = Characters[*c]; //untuk mencari karakter
		...
		GLfloat vertices[] = { //position agar ngegeser
...
{
	// Init Freetype (untuk membaca file ttf)
	FT_Library ft;
	if (FT_Init_FreeType(&ft)) {
		throw ("ERROR::FREETYPE: Could not init FreeType Library");
	}
	FT_Face face;
	if (FT_New_Face(ft, fontName, 0, &face)) { //baca font name
	...
	for (GLubyte c = 0; c < 128; c++) //membaca karakter ke 0-127
	{
		// Load character glyph
		if (FT_Load_Char(face, c, FT_LOAD_RENDER)) //disimpan dalam object face
		{
		...
		Character character = {
			texture,
			ivec2(face->glyph->bitmap.width, face->glyph->bitmap.rows), //simpan dalam map
