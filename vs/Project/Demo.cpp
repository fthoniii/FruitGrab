...
void Demo::Init()
{
	...
	for (int i = 0; i < 8; i++) {
		...
		platformSprite->SetSize(150, 32); //untuk merubah width dan height
...
void Demo::Update()
{
	...
	if (inputManager->IsKeyPressed("Jump") && !jump) {
		...
		gravity = 0.16f * ratio; //semakin besar gravitasi dia akan cepet turun
		yVelocity = 1.8f;
		jump = true;
		sound->Play(false); //dipanggil
	}

	if (y > 0) { // tidak dalam ground, sedang loncat
		yVelocity -= gravity; //kecepatan jump
	}
	else if (y < 0) {
		jump = false;
		yVelocity = 0;
