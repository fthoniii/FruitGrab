...
bool Input::IsKeyPressed(string name)
{
	...
	if (it != pressedKeyMap.end()) { //apakah sudah pernah ditekan atau belum
		// Found the key
		return it->second;
	}
	// Didn't find the key
...
{
	// Akan memasukkan ke dalam map, bahwa tombol itu sudah ditekan
...
{
	// tombol yang udah pernah ditekan dijadikan false, dan dimasukkan ke database bahwa tombol tersebut sudah pernah ditekan
	auto it = keyCodeNameMap.find(keyID);
	if (it != keyCodeNameMap.end()) {
		string mapName = it->second;
...
void Input::CloseGameController() //menutup koneksi IO ke gamepath 
{
	if (controller != nullptr) { // jika kontroller sudah pernah dibuka
