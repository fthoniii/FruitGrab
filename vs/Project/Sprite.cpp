...
void Sprite::Draw()
{
	...
	// aktifkan blending
	glEnable(GL_BLEND);
	// menggunakan transparasi gambar
...
void Engine::Sprite::Update(float deltaTime)
{
	...
	if (!enableAnimation || currentAnim == NULL || n == 1) //jika enable nya gak aktif maka gak dijalankan, jika animasi 1 maka gk ada animasi
		return;

	frameDuration += deltaTime; //sebagai counter untuk menghitung seberapa lama sebuah frame, jika sudah melampaui batas ganti ke frame selanjutnya

	if (frameDuration >= maxFrameDuration) { //kalo mau lambat animasi nya maxFrameDuration besar
		frameDuration = 0;
		frameIndex = (frameIndex < currentAnim->startFrameIndex || frameIndex >= currentAnim->endFrameIndex) ? currentAnim->startFrameIndex : frameIndex + 1; //? -> maka
	}
}
...
void Engine::Sprite::AddAnimation(string name, int startFrameIndex, int endFrameIndex)
{
	...
	animationData.push_back(a); //push_back itu sama kek add menambahkan data ke array
}

void Engine::Sprite::PlayAnim(string name) //memainkan animasi
{
	enableAnimation = true;
	if (currentAnim != NULL && name == currentAnim->name)return; //jika udah aktif tinggal di return
...
void Sprite::SetScore(int value) {
	score = value;
}
...
void Engine::Sprite::SetSpeed(float newSpeed) {
...
mat4 Sprite::CreateTransform()
{
	...
	transform = translate(transform, vec3(x, y, 0.0f)); //melalukan translasi untuk memindah posisi dari obek
	// Rotate sprite at center -> muter di tengah2 koordinat
	transform = translate(transform, vec3(0.5f * w, 0.5f * h, 0.0f));
	transform = rotate(transform, radians(degree), vec3(0.0f, 0.0f, 1.0f)); //merubah derajat ke radian, jadi ketika nanti mau manggil rotasi make derajat aja otomatis bakal diubah ke radian
	transform = translate(transform, vec3(-0.5f * w, -0.5f * h, 0.0f));
	// Scale sprite
	...
	transform = translate(transform, vec3(tx, ty, 0.0f)); //tx dan ty adalah width dan height
	transform = scale(transform, vec3(sx * w, sy * h, 1));

	return transform;
}

void Engine::Sprite::UpdateShaderData() //update jumlah kolom, baris, dan frameIndex
{
	shader->Use();
