...
{
	//prepare the normals
	RectVertexArray normals_box1 = GetNormals(vecs_box1); // mencari vektor normal, vektor yang tegak lurus dari permukaan
	RectVertexArray normals_box2 = GetNormals(vecs_box2);

	//results of P, Q // cari nilai minimal dan maksimum
...
Engine::Result Engine::SATCollision::getMinMax(RectVertexArray& vecs_box, vec2& axis) // menggunakan dot product
{
...
vec2 Engine::SATCollision::GetNormal(const vec2& v) // mencari normal
{
	return vec2(-v.y, v.x);
}
...
{
	return a.x * b.x + a.y * b.y; // rumus dot product
}